...
def cbam_module(input_feature, ratio=8):
    ...
    cbam_feature = layers.Activation('sigmoid')(layers.Add()([avg_pool, max_pool]))
    x = layers.Multiply()([input_feature, cbam_feature])
    ...
    cbam_feature = layers.Conv2D(1, 7, strides=1, padding='same', activation='sigmoid', kernel_initializer='he_normal', use_bias=False)(concat)
    return layers.Multiply()([x, cbam_feature])

def residual_block(input_tensor, filters, strides=1):
    x = layers.Conv2D(filters, 3, strides=strides, padding='same')(input_tensor)
...
def BetterNet(input_shape=(256, 256, 3), num_classes=1, dropout_rate=0.5):
    ...
    d1 = decoder_block(encoder.output, encoder.get_layer('block6a_expand_activation').output, 256, dropout_rate)
    d2 = decoder_block(d1, encoder.get_layer('block4a_expand_activation').output, 128, dropout_rate)
    d3 = decoder_block(d2, encoder.get_layer('block3a_expand_activation').output, 64, dropout_rate)
    d4 = decoder_block(d3, encoder.get_layer('block2a_expand_activation').output, 32, dropout_rate)
